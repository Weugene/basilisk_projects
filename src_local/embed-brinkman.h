...
#define cs_avg(a,i,j,k)							\
  ((a[i,j,k]*(1.5 + fs[i,j,k]) + a[i-1,j,k]*(1.5 + fs[i-1,j,k]))/	\
   (fs[i,j,k] + fs[i-1,j,k] + 3.))
...
#if dimension == 2
#define face_condition(fs_face, fs)						\
  (fs_face.x[i,j] > 0.5 && fs_face.y[i,j + (j < 0)] && fs_face.y[i-1,j + (j < 0)] &&	\
   fs[i,j] && fs[i-1,j])

foreach_dimension()
static inline double embed_face_gradient_x (Point point, scalar a, int i)
{
  int j = sign(fs_face.x[i,1] - fs_face.x[i,-1]);
  assert (fs[i] && fs[i-1]);
  if (face_condition (fs_face, fs))
    return ((1. + fs_face.x[i])*(a[i] - a[i-1]) +
	    (1. - fs_face.x[i])*(a[i,j] - a[i-1,j]))/(2.*Delta);
...
{
  int j = sign(fs_face.x[i,1] - fs_face.x[i,-1]);
  return face_condition (fs_face, fs) ?
    ((1. + fs_face.x[i])*cs_avg(a,i,0,0) + (1. - fs_face.x[i])*cs_avg(a,i,j,0))/2. :
    cs_avg(a,i,0,0);
}
...
#define face_condition(fs_face, fs)						\
  (fs_face.x[i,j,k] > 0.5 && (fs_face.x[i,j,0] > 0.5 || fs_face.x[i,0,k] > 0.5) &&	\
   fs_face.y[i,j + (j < 0),0] && fs_face.y[i-1,j + (j < 0),0] &&			\
   fs_face.y[i,j + (j < 0),k] && fs_face.y[i-1,j + (j < 0),k] &&			\
   fs_face.z[i,0,k + (k < 0)] && fs_face.z[i-1,0,k + (k < 0)] &&			\
   fs_face.z[i,j,k + (k < 0)] && fs_face.z[i-1,j,k + (k < 0)] &&			\
   fs[i-1,j,0] && fs[i-1,0,k] && fs[i-1,j,k] &&				\
   fs[i,j,0] && fs[i,0,k] && fs[i,j,k])
...
within the embedded boundary (for which *fs* is zero). */

#undef center_gradient
#define center_gradient(a) (fs_face.x[] && fs_face.x[1] ? (a[1] - a[-1])/(2.*Delta) : \
			    fs_face.x[1] ? (a[1] - a[])/Delta :		    \
			    fs_face.x[]  ? (a[] - a[-1])/Delta : 0.)
...
$0 < fs < 1$), *embed_geometry()* returns the area of the fragment,
...
{
  *n = facet_normal (point, fs, fs_face);
...
embedded boundary defined by *fs*, of unit normal vector *n*
...
foreach_dimension()
static inline double dirichlet_gradient_x (Point point, scalar s, scalar fs,
					   ...
					   double * coef)
{
  ...
  if (defined)
    for (int l = 0; l <= 1; l++) {
      ...
      y1 -= j;
#if dimension == 2
      if (fs_face.x[i + (i < 0),j] && fs_face.y[i,j] && fs_face.y[i,j+1] &&
	  fs[i,j-1] && fs[i,j] && fs[i,j+1])
	v[l] = quadratic (y1, (s[i,j-1]), (s[i,j]), (s[i,j+1]));
#else // dimension == 3
      double z = p.z + d[l]*n.z;
      int k = z > 0.5 ? 1 : z < -0.5 ? -1 : 0;
      z -= k;
      bool defined = fs_face.x[i + (i < 0),j,k];
      for (int m = -1; m <= 1 && defined; m++)
	if (!fs_face.y[i,j,k+m] || !fs_face.y[i,j+1,k+m] ||
	    !fs_face.z[i,j+m,k] || !fs_face.z[i,j+m,k+1] ||
	    !fs[i,j+m,k-1] || !fs[i,j+m,k] || !fs[i,j+m,k+1])
	  defined = false;
      if (defined)
...
double dirichlet_gradient (Point point, scalar s, scalar fs,
			   coord n, coord p, double bc, double * coef)
{
#if dimension == 2
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return dirichlet_gradient_x (point, s, fs, n, p, bc, coef);
#else // dimension == 3
  if (fabs(n.x) >= fabs(n.y)) {
    ...
    return dirichlet_gradient_y (point, s, fs, n, p, bc, coef);
  return dirichlet_gradient_z (point, s, fs, n, p, bc, coef);
#endif // dimension == 3
  return nodata;
}
...
coord embed_gradient (Point point, vector u, coord p, coord n)
{
  ...
  foreach_dimension() {
    ...
    if (dirichlet) {
      double val;
      dudn.x = dirichlet_gradient (point, u.x, fs, n, p, vb, &val);
    }
    else // Neumann
      dudn.x = vb;
    if (dudn.x == nodata)
...
void embed_force (scalar p, vector u, face vector mu, coord * Fp, coord * Fmu)
{
  ...
  foreach (reduction(+:Fpx) reduction(+:Fpy)
    ...
    if (fs[] > 0. && fs[] < 1.) {
      ...
      fprintf(ferr, "n=%g %g b=%g %g area=%g", n.x, n.y, b.x, b.y, area);
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  coord n = facet_normal (point, fs, fs_face), p;
  double alpha = plane_alpha (fs[], n);
  double area = plane_area_center (n, alpha, &p);
  ...
  if (dirichlet) {
    normalize (&n);
    grad = dirichlet_gradient (point, s, fs, n, p, grad, &coef);
  }
...
{
  if (!coarse(fs) || !coarse(fs,child.x))
    return coarse(s);
  #if dimension >= 2
  if (!coarse(fs,0,child.y) || !coarse(fs,child.x,child.y))
    return coarse(s);
  #endif
  #if dimension >= 3
  if (!coarse(fs,0,0,child.z) || !coarse(fs,child.x,0,child.z) ||
      !coarse(fs,0,child.y,child.z) ||
      !coarse(fs,child.x,child.y,child.z))
    return coarse(s);  
  #endif
...
void update_tracer (scalar f, face vector uf, face vector flux, double dt)
{
  ...
  foreach() {
    ...
    *f* in time. */
    ...
    $$
    Note that *fs_face* does not appear in the code below because *uf*
	...
	foreach_dimension()
	  if (fabs(uf.x[i]) > umax)
	    umax = fabs(uf.x[i]);
      double dtmax = Delta*fs[]/(umax + SEPS);
...
//    fs_face.x[] = 1.;
//#if TREE
//  fs.refine = embed_fraction_refine;
//
//  /**
//  For prolongation we cannot use the same function since the surface
//  fraction field *fs_face* is not necessarily defined for prolongation
//  cells. So we switch back to the default fraction refinement (which
//  is less accurate but only relies on *fs*). */
//
//  fs.prolongation = fraction_refine;
//  foreach_dimension()
//    fs_face.x.prolongation = embed_face_fraction_refine_x;
//
//  /**
...
//event defaults (i = 0 ) {
//  display ("draw_vof (c = 'fs', s = 'fs_face', filled = -1, "
