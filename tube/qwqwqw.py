...
try:
  print('trying to get active source')
  my_source = GetActiveSource()
except:
  print("can't get active source")
  sys.exit()
#     sys.exit()
...
if not noPic:
  for i in timeList:
      ...
      cellDatatoPointData0 = CellDatatoPointData(Input=my_source)
#         cellDatatoPointData0.CellDataArraytoprocess = ['f', 'fs', 'l', 'l2', 'omega', 'u.x']

      # create a new 'Calculator'
      calculator1 = Calculator(Input=cellDatatoPointData0)
      calculator1.ResultArrayName = 'meanX'
      calculator1.Function = 'coordsX*(1-f)'
      ...
      passArrays1.PointDataArrays = [ 'f', 'meanX']

      # update the view to ensure updated data information
      spreadSheetView1.Update()

      ss_data = Fetch(passArrays1)
      x_sum = ss_data.GetPointData().GetArray('meanX').GetValue(0)
      bubble_volume = lDomain**3 - ss_data.GetPointData().GetArray('f').GetValue(0)
      x_mean = x_sum/bubble_volume
      print ("Fetched data: x_sum:", x_sum, " bubble volume:", bubble_volume, " x_mean:", x_mean)
...
#         length = len_max - len_min
      ...
      bounds = [x_mean - 1.2, x_mean + 1.2, -0.5, 0.5, -0.5, 0.5  ]
      print("bounds_rude=", bounds)
      center = [(bounds[0] + bounds[1])/2, (bounds[2] + bounds[3])/2,(bounds[4] + bounds[5])/2]
      print('center_rude=', center)
      len_bub = bounds[1] - bounds[0]
...
#****************** LAMBDA2 inside a bubble  ********************
      ...
      print ("Showing contour1 and hiding contour2.. "),
...
#****************** CONTOUR1(f) AND TRACERS ********************
      print('hiding contour1, contour2, contour5..')
      Hide(contour1, renderView1)
...
#**************************************************************
      ...
      for k,sl in enumerate(np.linspace(bounds[0]+0.1, bounds[1]-0.1, nslices)):
          ...
          renderView2.CameraParallelScale = 1.5
...
#             print ("Showing contour1 renderView2.. "),
#             Show(contour1, renderView2, 'GeometryRepresentation') # Weugene: ??? was commented deliberately
#             print ("end")
          # show data from contour1
          print ("Showing contour1.. "),
