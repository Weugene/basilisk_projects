...
static void sweep_x (scalar c, scalar cc)
{
  ...
  */
  ...
  foreach() {
    ...
    //if ( fs[]==1 && f[]<1 ) fprintf(ferr, "fs=%g f=%g deltaflux=%g deltauf_x=%g deltac[]=%g \n", fs[], f[], dt*(flux[] - flux[1])/(cm[]*Delta +SEPS), dt*cc[]*(uf.x[1]-uf.x[])/(cm[]* Delta +SEPS), dt*(flux[] - flux[1] + cc[]*(uf.x[1] - uf.x[]))/(cm[]*Delta + SEPS));
    scalar t, tflux;
