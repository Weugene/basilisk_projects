...
// vector with 3 variable doubles
...
//    UMEAN = G*sq(0.5)/(8*Mu1);
    ...
    fprintf(ferr,"BP:             eta_s=%g,     DT=%g\n"
                 ...
                 "OUTPUT:         dt_vtk=%g number of procs=%d snapshot_i=%d\n"
...
//Inflow
u.n[right] = dirichlet(uexact(x,y,z)*(1 - fs[]) - Ubmean);
u.r[right] = dirichlet(0);
u.t[right] = dirichlet(0);
p[right] = neumann(0);
pf[right] = neumann(0);
// p[right] = neumann(-rho1*fs[]*(u.x[] - target_Uv.x[])/eta_s);
// pf[right] = neumann(-rho1*fs[]*(u.x[] - target_Uv.x[])/eta_s);
f[right] = dirichlet(1);
fs[right] = neumann(0);

#define sqq(x) (pow(x,10))
void geometry(scalar fs, int i, double t)
{
    //Channel
    vertex scalar phi[];
    double r2;
...
//	fprintf(ferr, "a=%g, b=%g, c=%g\n", a, b, c);
	double x_needle = x_init + 0.5*(l_bub + 2*r_bub) + 4 - Ubmean*t;
    fprintf(ferr, "i= %d t= %g Ubmean= %g l_bub= %g r_bub= %g x_init= %g x_needle= %g\n",
                   i, t, Ubmean, l_bub, r_bub, x_init, x_needle);
    foreach_vertex() {
	    r2 = sq(y) + sq(z);
	    phi[] = intersection(sq(r_innner) - r2, x - x_needle); //difference(a,b)     min(a,-(b))
        phi[] = union(r2 - sq(0.5), phi[]);
//        phi[] = union(r2 - sq(0.5), sq(1) - sqq((x - lDomain - Ubmean*t)/a) - sqq(y/b) - sqq(z/c));
    }
    boundary ({phi});
    fractions (phi, fs);
}
...
void bubble(scalar f)
{
    ...
    foreach_vertex() {
        if (ellipse_shape)
            phi[] = sq((x - x_init)/(0.5*l_bub)) + sq(y/r_bub) + sq(z/r_bub) - sq(1);
        else if (cylinder_shape)
        {
...
event init (t = 0) {
    if (!restore (file = "restart")) {
        ...
        fprintf(ferr, "In frame of the bubble reference\n");
        double volumeg = 0, dvr;
        foreach(reduction(+:Ubmean) reduction(+:volumeg)){
            dvr = dv()*(1 - fs[])*(1 - f[]); // gas cell
            volumeg += dvr;
            Ubmean += u.x[]*dvr;
        }
        Ubmean /= volumeg;
        ...
        foreach(reduction(+:Ubmean) reduction(+:dUbmean)
                reduction(+:volumes) reduction(+:volumeg)){
            dvs = dv()*fs[]; // solid cell
            dvg = dv()*(1 - fs[])*(1 - f[]); // gas cell
            volumes += dvs;
            volumeg += dvg;
            Ubmean += u.x[]*dvs;
            dUbmean += u.x[]*dvg;
    ...
    fprintf(ferr, "Ubmean=%g dUbmean=%g", Ubmean, dUbmean);
}
...
event end_timestep (i++){
    ...
    foreach(reduction(+:dUbmean) reduction(+:volumeg)) {
        dvg = dv()*(1 - fs[])*(1 - f[]);
        volumeg += dvg;
        dUbmean += u.x[]*dvg;
    }
    ...
    foreach() u.x[] -= dUbmean;
    boundary((scalar *){u});
    foreach_face() uf.x[] -= dUbmean;
    boundary ((scalar *){uf});
    target_Uv = define_v();
    target_U = target_Uv;
    fprintf(ferr, "Ubmean=%g dUbmean=%g\n", Ubmean, dUbmean);
}
/**
## Counting bubbles
...
event logfile (i +=10)
{
    ...
    x_mean /= volumeg; velamean = sqrt(sq(vel_bubx) + sq(vel_buby) + sq(vel_bubz)); length = x_max - x_min;
...
//event snapshot (t += 1e-1)
{
    char name[80];
    scalar ppart[];
    scalar omega[]; vorticity (u, omega);
    scalar l2[]; lambda2 (u, l2);
...
{
    fprintf(ferr, "NO OUTPUT in sake of reduction memory consumptions\n");
    char subname[80]; sprintf(subname, "tube_needle");
