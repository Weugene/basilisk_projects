...
#define ADAPT_SCALARS {gf.x, gf.y, p}
#define ADAPT_EPS_SCALARS {feps, feps, peps}
...
event init (t = 0) {
    ...
    if (!restore (file = "restart")) {
        ...
        do {
            ...
            boundary ({f, u});
            gradients ({f}, {gf});
        }while (adapt_wavelet({gf.x, gf.y}, (double []){feps, feps},
...
//    scalar err[]; foreach() err[] = fabs(p[] - Psol[]); //be careful with kappa, mu. They can be const unity
    output_vtu_MPI( (scalar *) {l, f, rho, p}, (vector *) {u}, subname);
}
