...
double Vd, Vdst, deq, dst = 0.2, rst = 0.1, r_bub, l_bub, ok;
double RhoR, MuR;
double Rho1, Rho2;
double Mu1, Mu2;
double Sigma;
...
Cases cases[23]={
        ...
        {0.0230, 0.1179e-10, 1.580, 1.944, 0.054}  // 22 Air-Water // 0.2179-9
};
...
//    relative_residual_viscous = true;
    ...
    if ((bubcase >= 8 && bubcase <= 12) || bubcase >= 18) { // Water
        Rho1 = 997, Rho2 = 1250;//!!! Changed
        Mu1 = 0.88e-3, Mu2 = 550e-3;
        Sigma = 37.2e-2;//!!! Changed water/transformer oil https://en.wikipedia.org/wiki/Surface-tension_values
        diam_tube = 514e-6;
        dt_vtk = 1e-1;
        lDomain = 20;
    }else { // Glycerol
...
//    Umean = G*sq(0.5)/(8*Mu1);
    ...
    if (ellipse_shape || rst < 0.9*ok) {
        r_bub = min(rst, 0.9*ok);
...
{
    const double R = 1;
    const double d = R/sqrt(3);
    coord p0[3] = {
            {0, R, -d},{0, -R, -d},{0, 0, 2.0*R/sqrt(3.0)}
    };
    //Channel
    vertex scalar phi[];
    foreach_vertex() {
...
//        phi[] = HUGE;
        for (int i=0; i<3; i++)
            phi[] = max(phi[], sq(R) - sq(y - p0[i].y) - sq(z - p0[i].z) );
    }
    boundary ({phi});
    fractions (phi, fs);
}
