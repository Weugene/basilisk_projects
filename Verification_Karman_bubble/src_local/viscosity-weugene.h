...
//        foreach() fprintf(ferr, "target_U %g %g U_solid %g %g nsol= %g %g\n", target_U.x[], target_U.y[], U_solid.x[], U_solid.y[], n_sol.x[], n_sol.y[]);
    ...
    foreach (reduction(max:maxres)) {
        foreach_dimension(){
            res.x[] = r.x[]
                      PLUS_BRINKMAN_RHS
                      - lambda.x*u.x[] + frhs*dt*divtauu.x[]/(Delta*rho[]);
...
//        fprintf(ferr, "norm(u)>0.001\n");
//        foreach() if (norm(u)>0.001) fprintf(ferr, "***tU: %g %g u: %g %g dbp: %g %g rhs: %g %g fs: %g dbpN: %g %g r: %g %g Nrhs: %g %g res: %g %g \n", target_U.x[], target_U.y[], u.x[], u.y[], dbp.x[], dbp.y[], total_rhs.x[], total_rhs.y[], fs[], fbp*(- (u.x[] - target_U.x[])/eta_s), fbp*(- (u.y[] - target_U.y[])/eta_s), r.x[], r.y[], delete_me.x[], delete_me.y[], res.x[], res.y[]);
//        fprintf(ferr, "fs[]>0\n");
//        foreach() if (fs[]>0.1)      fprintf(ferr, "---tU: %g %g u: %g %g dbp: %g %g rhs: %g %g fs: %g dbpN: %g %g r: %g %g Nrhs: %g %g res: %g %g \n", target_U.x[], target_U.y[], u.x[], u.y[], dbp.x[], dbp.y[], total_rhs.x[], total_rhs.y[], fs[], fbp*(- (u.x[] - target_U.x[])/eta_s), fbp*(- (u.y[] - target_U.y[])/eta_s), r.x[], r.y[], delete_me.x[], delete_me.y[], res.x[], res.y[]);
#endif
...
mgstats viscosity (struct Viscosity p){
    ...
    scalar rho = p.rho;
    restriction ({mu,rho});
//    foreach() if (fs[]>0.1) fprintf(ferr, "!!!tU: %g %g u: %g %g r: %g %g dbp: %g %g rhs: %g %g fs: %g dbpN: %g %g\n", target_U.x[], target_U.y[], u.x[], u.y[], r.x[], r.y[], dbp.x[], dbp.y[], total_rhs.x[], total_rhs.y[], fs[], fbp*(- (u.x[] - target_U.x[])/eta_s), fbp*(- (u.y[] - target_U.y[])/eta_s));
