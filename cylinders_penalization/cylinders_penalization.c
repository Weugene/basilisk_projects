...
//fprintf(ferr, "Phi=%g vol=%g\n", Phi, s.volume);
//foreach() if (dv() <= 0. || u.x[] == nodata) {fprintf(ferr, "dv=%g u.x=%g Delta=%g x=%g y=%g cm=%g\n", dv(), u.x[], Delta, x, y, cm[]);}
...
//#else
//adapt_wavelet ({cs,u}, (double[]){1e-2,2e-3,2e-3}, maxlevel);
//#endif
cylinder (cs, fs);
}
#if 0
...
vorticity (u, omega);
foreach() l[] = level;
        output_vtu_MPI( (scalar *) {l, omega, cs, p}, (vector *) {u}, subname, 0.1*L0);
}
//#include "vtknew.h"
