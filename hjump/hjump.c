...
double Q = 5.e-5;//cm^3/s Q=v*S
#define v_in 4.0*Q/(pi*sq(diam))//inlet velocity
double hjet = 5e-2;//meter
double length = 4.55e-2;//4.1e-3
double hbreak = 1.0e-2, hhump=10e-3;
double Tac = 1e-1;

double SIGMA = 1e-3;//30-70e-3
double MUL = 1e-3, MUG = 18.5e-4;
...
event init (t = 0) {
	if (!restore (file = "restart")) {
		...
		refine( fabs(y - f_surf(x,y,0,0)) < diam && level < maxlevel );
...
event vtk_file (t+=0.01; t<10){
	...
	scalar l[];
	vorticity (u, omega);
	foreach() foreach_dimension() uliq.x[] = f[]*u.x[];
	foreach() {l[] = level; omega[] *= 1 - fs[];}
	output_vtu_MPI( (scalar *) {fs, f, omega, p, l}, (vector *) {u, uliq, a}, subname, 0.0);
}

event adapt (i++) {
	foreach() foreach_dimension() uliq.x[] = f[]*u.x[];
	adapt_wavelet ({f, fs, uliq}, (double[]){1e-3, 1e-4, 1e-2, 1e-2, 1e-2}, maxlevel=maxlevel, minlevel=minlevel);
	fs.refine = fs.prolongation = fraction_refine;
	boundary({fs});
}
